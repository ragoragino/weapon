...
fn main() {
    ...
    let cfg = parse_config(&cfg_json).
        expect("Failed parsing configuration");
    ...
    match cfg.mode {
        Mode::ClientServer => {
            ...
            run_client_server(shutdown_rx, &cfg)
        ...
        Mode::P2P => {
            ...
            run_p2p(shutdown_rx, &cfg)
                .expect("Unable to run VPN in P2P mode.");
        },
    }
}
...
fn run_client_server(shutdown_rx: tokio::sync::mpsc::Receiver<()>,
    cfg: &ClientServerConfiguration) -> Result<(), Box<dyn std::error::Error>> {
    // TODO
    Ok(())
}

fn run_p2p(mut shutdown_rx: tokio::sync::mpsc::Receiver<()>,
    cfg: &P2PConfiguration) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tunnel = match cfg.tunnel_cfg.protocol {
        TransportLayerProtocol::TCP => Box::new(TCPConnector::new(cfg.tunnel_cfg)?) 
            as Box<dyn Connector>,
        TransportLayerProtocol::UDP => Box::new(UDPConnector::new(cfg.tunnel_cfg, 
            runtime.handle().clone())?) as Box<dyn Connector>,
    };

    let lan = match cfg.lan_cfg.mode {
        LanDeviceMode::TUN => {
            let device_cfg = cfg.lan_cfg.tun_cfg.ok_or(Box::new(
                SetupError::UnexpectedError("missing TUN configuration".into()),
            ))?;
            let device = Arc::new(TUNDevice::new(device_cfg, runtime.handle().clone())?);
            Box::new(TUNConnector::new(cfg.lan_cfg, 
                runtime.handle().clone(), device)?) as Box<dyn Connector>
        },
        LanDeviceMode::TAP => {
            let device_cfg = cfg.lan_cfg.tap_cfg.ok_or(Box::new(
                SetupError::UnexpectedError("missing TAP configuration".into()),
            ))?;
